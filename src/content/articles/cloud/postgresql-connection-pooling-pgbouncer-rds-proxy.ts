...
export const article: Article = {
  ...
  body: [
    ...
    { type: "p", text: "PgBouncer is a common choice. The mode you pick changes the behavior of the app." },
    ...
    { type: "p", text: "This is common with ORMs. Many use server-side prepared statements by default. Before enabling transaction pooling, check the driver settings your app uses." },
    ...
    { type: "callout", variant: "warning", title: "Pinning can stop the pooling benefit", text: "When RDS Proxy sees state that must stay tied to a backend, it can pin the client to one connection. That protects app behavior. It also means that connection is no longer shared for that client." },
    { type: "p", text: "Pinning is a quiet problem. The app still works. The proxy is just doing less multiplexing. Watch the pinning metrics. If pinning is high, find which app behavior triggers it." },
    ...
    { type: "p", text: "Replication and change-data-capture connections also need headroom. Their interaction with general connection limits can differ between PostgreSQL versions and rollout types." },
    { type: "p", text: "That matters because a busy app can use the headroom you thought was reserved for replication. The result can be lag at the same time as user traffic is high." },
    ...
    { type: "p", text: "Keep the app pool too. The app pool reduces repeated handshakes. The fleet pool reduces the number of real database connections. They solve different problems." },
    ...
    { type: "p", text: "Postgres is not wrong for using a process per connection. That design gives useful isolation. The lesson is that connection count becomes a real capacity limit as an app grows. Pooling manages that limit. The pooling mode then decides which app assumptions still hold." },
  ],
  faq: [
